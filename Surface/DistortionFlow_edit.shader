Shader "Custom/Surface/DistortionFlow_edit" 
{
	Properties
	{
	...
	SubShader
	{
		...
		void surf (Input IN, inout SurfaceOutputStandard o) 
		{
			...
			flow.xy = flow.xy * 2 - 1;
			flow *= _FlowStrength;
			//flowVector = (0, 0); // Uncomment to not swirl the grid pattern and preserve it
			float noise = tex2D (_FlowMap, IN.uv_MainTex).a;
			float time = _Time.y * _Speed + noise;
			float2 jump = float2 (_UJump, _VJump);

			float3 uvwA = FlowUVW (
				IN.uv_MainTex, flow.xy, jump, time, _Tiling, _FlowOffset, false
			);
			float3 uvwB = FlowUVW (
				IN.uv_MainTex, flow.xy, jump, time, _Tiling, _FlowOffset, true
			);
